...
module.exports = class Log {
  ...
  constructor(row) {
    ...
    this.dateOfEvent = new Date(row.date_of_event).toLocaleDateString('en-US');
  ...
  static async find() {
    const { rows } = await pool.query(
      'SELECT id, recipe_id, cast(date_of_event + 1 AS text) as date_of_event, notes, rating FROM LOGS'
    );
